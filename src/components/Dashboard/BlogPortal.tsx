...
export default function BlogPortal({ onBack, onSelectTemplate }: BlogPortalProps) {
  ...
  useEffect(() => {
    if (activeArticle) {
      ...
      if (metaDesc) metaDesc.setAttribute('content', activeArticle.desc);
    } else {
      document.title = 'Tips, Panduan & Inspirasi Pernikahan Digital - Blog invitationbuilder.net';
      const metaDesc = document.querySelector('meta[name="description"]');
      if (metaDesc) metaDesc.setAttribute('content', 'Pusat edukasi pernikahan digital terpercaya.');
    }
  }, [activeArticle]);
  ...
  const filteredArticles = articles.filter(art => (selectedCategory === 'Semua' || art.category === selectedCategory) && (searchQuery === '' || art.title.toLowerCase().includes(searchQuery.toLowerCase()) || art.desc.toLowerCase().includes(searchQuery.toLowerCase())));
    ...
    <div className="min-h-screen bg-[#090c15] text-white font-sans selection:bg-blue-500/30">
      ...
      <main className="max-w-[1440px] mx-auto px-4 md:px-6 py-6">
        <div className="grid grid-cols-1 xl:grid-cols-[160px_1fr_160px] gap-6 items-start">
          ...
          {/* LEFT SIDEBAR (Stacked Skyscraper 160x600 Ads) */}
            ...
            {activeArticle ? (
              <article className="space-y-8 animate-in fade-in duration-200">
                ...
                <div className="space-y-4">
                  ...
                  <h1 className="text-2xl md:text-4xl font-extrabold text-white font-cinzel leading-tight">{activeArticle.title}</h1>
                  <div className="flex items-center space-x-4 text-slate-500 text-[10px] font-bold">
                    <span className="flex items-center space-x-1"><User className="w-3.5 h-3.5" /> <span>Tim Spesialis InviteStudio</span></span>
                    <span className="flex items-center space-x-1"><Clock className="w-3.5 h-3.5" /> <span>{activeArticle.readTime} Bacaan</span></span>
                    <span>•</span>
                ...
                <div className="bg-blue-950/20 border-l-4 border-blue-500 p-4 rounded-r-2xl text-xs text-slate-300 italic leading-relaxed">"{activeArticle.desc}"</div>
                ...
                {activeArticle.faqs && activeArticle.faqs.length > 0 && (
                  <div className="bg-slate-950/60 border border-slate-900 rounded-3xl p-6 md:p-8 space-y-4">
                    <h3 className="text-xs font-extrabold tracking-wider uppercase text-[#D4AF37] flex items-center space-x-2 font-cinzel">
                      <Sparkles className="w-4 h-4" /> <span>Pertanyaan Umum (FAQ)</span>
                    </h3>
                    <div className="space-y-4">
                      {activeArticle.faqs.map((faq, i) => (
                        <div key={i} className="space-y-1.5 text-xs bg-slate-900/40 p-4 rounded-xl border border-slate-900">
                          <h4 className="font-extrabold text-white flex items-start space-x-1.5"><span className="text-blue-400">Q:</span><span>{faq.q}</span></h4>
                          <p className="text-slate-400 font-light leading-relaxed pl-5"><strong className="text-amber-500 font-normal">A:</strong> {faq.a}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-gradient-to-r from-[#0d0f1e] to-slate-950 border border-slate-900 rounded-3xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl relative overflow-hidden">
                  <div className="space-y-2 max-w-md text-left">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed font-light">Mulai rancang undangan digital impian Anda di InviteStudio secara gratis.</p>
                  </div>
                  <button onClick={() => onSelectTemplate('premiumIndonesianFloral')} className="px-6 py-3.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold transition-all flex items-center space-x-2 shrink-0 cursor-pointer border-0 shadow-lg shadow-blue-600/20">
                    <Sparkles className="w-4.5 h-4.5" /> <span>Buat Undangan Gratis</span>
                  </button>
                </div>
              </article>
            ) : (
              <div className="space-y-8 animate-in fade-in duration-200">
                ...
                <div className="text-center max-w-2xl mx-auto space-y-4">
                  <div className="inline-flex items-center space-x-2 px-3 py-1 bg-blue-500/10 border border-blue-500/25 text-blue-400 rounded-full text-[10px] font-bold tracking-widest uppercase">
                    <BookOpen className="w-3.5 h-3.5" /> <span>Pusat Edukasi &amp; Tips Pernikahan Digital</span>
                  </div>
                  <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-white font-cinzel">Artikel &amp; Panduan Pernikahan Digital</h1>
                  <p className="text-xs text-slate-400 leading-relaxed font-light">Dapatkan tips persiapan pernikahan, susunan acara resepsi, strategi hemat biaya nikah, hingga rekomendasi lagu romantis dari tim spesialis InviteStudio.</p>
                </div>
                ...
                <div className="space-y-4 bg-slate-950/60 border border-slate-900 rounded-2xl p-4">
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Cari topik artikel (contoh: rsvp, lagu, anggaran, kata-kata)..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-slate-900/80 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors" />
                  </div>
                  <div className="flex items-center space-x-2 overflow-x-auto pb-1 scrollbar-none">
                    {categories.map((cat) => (
                      <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3.5 py-1.5 rounded-xl text-[11px] font-bold whitespace-nowrap transition-all cursor-pointer border ${selectedCategory === cat ? 'bg-blue-600 border-blue-500 text-white shadow-md shadow-blue-600/20' : 'bg-slate-900 border-slate-800 text-slate-400 hover:text-white hover:border-slate-700'}`}>{cat}</button>
                    ))}
                  </div>
                </div>
                ...
                {filteredArticles.length === 0 ? (
                  <div className="text-center py-16 text-slate-500 text-xs font-light">Tidak ada artikel yang cocok dengan pencarian Anda.</div>
                ) : (
                  <div className="space-y-5">
                    {filteredArticles.map((art, i) => (
                      <React.Fragment key={art.slug}>
                        <div onClick={() => handleArticleClick(art.slug)} className="bg-slate-950/40 border border-slate-900 hover:border-slate-800 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-start gap-5 hover:-translate-y-0.5 transition-all duration-300 cursor-pointer text-left group">
                          <div className="space-y-3.5 flex-1 min-w-0">
                            ...
                            <h2 className="text-base md:text-lg font-black text-white group-hover:text-blue-400 transition-colors font-cinzel leading-snug">{art.title}</h2>
                            <p className="text-xs text-slate-400 leading-relaxed font-light line-clamp-2">{art.desc}</p>
                            <div className="flex items-center space-x-4 text-[9px] text-slate-500 font-bold uppercase tracking-wider pt-1">
                              <span className="flex items-center space-x-1"><Clock className="w-3.5 h-3.5" /> <span>{art.readTime} Baca</span></span>
                            </div>
                          </div>
                          <div className="w-8 h-8 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-400 group-hover:text-blue-400 group-hover:bg-blue-500/10 shrink-0 self-center hidden md:flex"><ChevronRight className="w-4 h-4" /></div>
                        </div>
          ...
          {/* RIGHT SIDEBAR (Stacked Skyscraper 160x600 Ads) */}
          <aside className="hidden xl:flex flex-col gap-6 shrink-0 sticky top-20">
